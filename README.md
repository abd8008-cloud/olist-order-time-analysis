...
لتأكيد صحة النتائج، تم إعادة نفس التحليل باستخدام Python (Pandas) على نفس البيانات الخام - النتائج تطابقت تماماً م�[...]
...
**الاستنتاج:** بطاقة الائتمان هي الطريقة المهيمنة بوضوح (أكثر من 73% من إجمالي المدفوعات) - أي استراتيجية تركز �[...]
...
**الاستنتاج:** São Paulo تهيمن بوضوح على قاعدة العملاء (ضعف المدينة الثانية تقريباً) - أي قرار توسع لوجستي أو تسوي[...]

**ملاحظة تقنية:** تم إجراء هذا التحليل عبر Kaggle Notebooks (بدلاً من Google Colab) للاستفادة من الربط المباشر والدائم بالب[...]
...
**الاستنتاج:** الفرق بين المقياسين (137.75 مقابل 120.65) يعكس أن نسبة من الطلبات تحتوي على أكثر من منتج واحد. عند اتخ[...]

## Additional Analysis: Customer Lifetime Spending
سؤال إضافي: ما هو متوسط إنفاق العميل الواحد عبر كل طلباته (وليس فقط الطلب الواحد)؟

هذا التحليل يتطلب ربط ثلاثة جداول معاً (Orders + Customers + Payments) باستخدام merge (ما يعادل JOIN في SQL).
...
# ربط الناتج بالمدفوعات
full_df = merged_df.merge(payments_df, on='order_id')

# حساب إجمالي إنفاق كل عميل، ثم متوسط ذلك عبر كل العملاء
customer_spending = full_df.groupby('customer_unique_id')['payment_value'].sum()
average_customer_value = customer_spending.mean()

print("متوسط إنفاق العميل عبر كل طلباته:", average_customer_value)
```
...
| متوسط سعر المنتج الواحد | 120.65 |
| متوسط قيمة الطلب الكامل | 137.75 |
| **متوسط إنفاق العميل عبر كل طلباته** | **166.59** |

**الاستنتاج:** الارتفاع التدريجي بين المقاييس الثلاثة يعكس أن جزءاً من العملاء يكررون الشراء أكثر من مرة. هذا الرقم (166.59) هو الأهم لاتخاذ قرارات التسويق - فهو يحدد السقف المعقول للإنفاق على اكتساب عميل جديد (Customer Acquisition Cost) مع الحفاظ على الربحية.
